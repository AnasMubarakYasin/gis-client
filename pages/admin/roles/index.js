...
export default function Roles(props) {
  ...
  const handleCardClick = (index) => (event) => {
    if (removeMode) {
      ...
      const role = roles[index];
      if (role) {
        const copy = { ...role, showPassword: false };
    ...
    if (isRemovingSuccess) {
      setSnack((prev) => ({
        ...
        message: (
          ...
          >
            Success Remove {dataRemoving.length} role.
          </Alert>
        ),
      }));
      refetch();
  ...
  useEffect(() => {
    if (isUndoingSuccess) {
      setSnack((prev) => ({
        ...
        message: (
          <Alert elevation={6} severity="success">
            Success Undo {dataRemoving.length} role.
          </Alert>
        ),
      }));
      refetch();
      ...
      >
        <DialogTitle display="flex" alignItems="center">
          <Typography component="div" variant="h6" sx={{ flexGrow: 1 }}>
            {dialogValue.id ? "Update" : "Create"} Role
          </Typography>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are your sure want to Remove {removeList.length} Role?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          ...
          <Button onClick={handleRemoverole}>Yes</Button>
        </DialogActions>
      </Dialog>
      <Snackbar
        key={snack.id}
